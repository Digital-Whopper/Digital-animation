export default async function handler(req, res) {
  ...
  try {
    ...
    // 1. Live Website HTML Crawling
    try {
      const response = await fetch(targetUrl, {
        headers: { 'User-Agent': 'DigitalWhopper-SEO-Bot/1.0' },
      ...
      console.log('Crawling error or blocked:', err.message);
    }

    // 2. Strict SEO Health Score Calculation based on Real HTML Tags
    ...
    if (fetchSuccess && htmlContent) {
      ...
      seoChecks.h1Count = h1Matches ? h1Matches.length : 0;
      if (seoChecks.h1Count === 1) {
        calculatedScore += 20; // Perfect H1
      } else if (seoChecks.h1Count > 1) {
        calculatedScore += 8;  // Multiple H1 penalty/warning
      }
      ...
      // If site blocks external crawler, give an average range score
      calculatedScore = 72;
    }

    // Ensure score bounds
    if (calculatedScore > 96) calculatedScore = 96;
    if (calculatedScore < 55) calculatedScore = 55;
    ...
    if (GOOGLE_SHEET_WEB_APP_URL) {
      try {
        ...
        console.log('Sheet sync error:', sheetErr.message);
      }
    }
