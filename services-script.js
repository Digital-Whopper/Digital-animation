...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (targetGrid) {
    targetGrid.innerHTML = servicesList.map(s => `
      <div class="service-card-core" onclick="window.location.href='services-details/?id=${s.id}'" style="cursor: pointer;">
        <div class="card-icon-slot">${s.icon}</div>
        <h3>${s.title}</h3>
        <p>${s.desc}</p>
        <span class="card-action-trigger" onclick="event.stopPropagation(); window.location.href='services-details/?id=${s.id}'" style="color: #0052ff; font-weight:600; display:inline-block; margin-top:12px; cursor: pointer;">
          Learn More ✦
        </span>
      </div>
    `).join('');
